<script lang="ts">
  ...
  const load_component = async () => {
    try {
      ...
      let componentSource = tool.source;
      // Normalize source path if it comes from DB with full path
      if (componentSource.includes("/src/tools/")) {
        const parts = componentSource.split("/");
        const filename = parts[parts.length - 1];
        componentSource = filename.replace(".svelte", "");
      }

      const component_module = await import(
        `../tools/${componentSource}.svelte`
      );
...
{:else if loading_state === "error"}
  ...
  >
    <div class="flex flex-col items-center space-y-4 max-w-md text-center">
      <div
        class="h-12 w-12 text-red-400 flex items-center justify-center text-2xl"
      >
        ⚠
      </div>
      <div class="space-y-2">
