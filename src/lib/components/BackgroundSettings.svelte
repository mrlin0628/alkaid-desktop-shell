<script lang="ts">
  ...
  import { backgroundPreferences, updateBackgroundPreference, resetToDefaults } from '../stores/backgroundPreferences';

  let showSettings = $state(false);

  function handleVideoUploaded(event: CustomEvent<{ url: string; filename: string }>) {
  ...
  function setColorBackground(color: string) {
    // Ensure we switch to color mode and update the color
...
<button
  class="fixed top-4 right-4 z-50 px-4 py-2 bg-cyber-purple bg-opacity-20 border border-cyber-purple rounded text-cyber-purple hover:bg-opacity-30 transition-all duration-200"
  onclick={() => showSettings = !showSettings}
>
  {showSettings ? 'Hide' : 'Background'} Settings
</button>

<!-- Settings Panel -->
{#if showSettings}
  <div class="fixed top-16 right-4 w-96 max-h-[80vh] overflow-y-auto bg-black bg-opacity-90 border border-cyber-purple rounded-lg p-6 z-50">
    <h3 class="text-cyber-purple font-semibold text-lg mb-4">Background Settings</h3>
    ...
    <div class="mb-6 p-3 bg-cyber-dark border border-gray-600 rounded text-sm">
      ...
      {#if $backgroundPreferences.source}
        <p><strong>Video:</strong> {$backgroundPreferences.source.split('/').pop()}</p>
      {/if}
    ...
    <div class="mb-6">
      ...
      <div class="flex gap-2">
        <button
          class="px-3 py-2 rounded border transition-all duration-200"
          class:bg-cyber-blue={$backgroundPreferences.type === 'color'}
          class:bg-opacity-20={$backgroundPreferences.type === 'color'}
          class:border-cyber-blue={$backgroundPreferences.type === 'color'}
        ...
        <button
          ...
          class:border-cyber-blue={$backgroundPreferences.type === 'video'}
        ...
        <div class="grid grid-cols-6 gap-2">
          {#each presetColors as color}
            <button
              ...
              class:border-cyber-blue={$backgroundPreferences.color === color}
              class:border-gray-600={$backgroundPreferences.color !== color}
